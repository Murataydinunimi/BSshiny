...
SPXcalls = cbind(SPXcalls,Sppricenum[1:122]) # 1:122 because the app presents stocks of the last month only.
...
SPXcalls = SPXcalls[,c(5,1,6,4,2,3)] #reorder the columns

#get the stock price for puts


SPXputs<-SPXputs[-c(2,3,4,5,6,7,10)]
riskfreeR<-matrix(rep(0.015,165))
SPXputs<-cbind(SPXputs,riskfreeR)
SPXputs$Lastradetime<-as.Date(SPXputs$LastTradeTime)
SPXputs<-SPXputs[,-2]
SPXputs = cbind(SPXputs,Sppricenum[1:165])
SPXputs = cbind(SPXputs,maturityinyears[1:165])
