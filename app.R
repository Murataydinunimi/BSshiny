...
#since maturity is not given for options above, I created a fake maturity from S&P prices to use for the options above.


getSymbols("FTSEMIB.MI",from="2014-01-01",to="2016-12-31")
timeofobservation<-as.numeric(index(Price))
daysInOneYear<-365 # actual convention
maturityinyears<-diff(timeOfObervation)/(daysInOneYear)
